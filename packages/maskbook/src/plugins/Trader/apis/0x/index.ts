...
export async function swapQuote(request: SwapQuoteRequest) {
    ...
    if (request.slippagePercentage)
        params.set('slippagePercentage', new BigNumber(request.slippagePercentage).dividedBy(BIPS_BASE).toString())
    if (request.buyTokenPercentageFee)
        params.set('buyTokenPercentageFee', new BigNumber(request.buyTokenPercentageFee).dividedBy(100).toString())
