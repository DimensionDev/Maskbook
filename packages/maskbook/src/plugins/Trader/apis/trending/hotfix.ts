...
export function resolveAlias(keyword: string, dataProvider: DataProvider) {
    ...
    if (dataProvider === DataProvider.IDEX) return keyword
...
export function resolveCoinId(keyword: string, dataProvider: DataProvider) {
    ...
    if (dataProvider === DataProvider.IDEX) return KEYWORK_ID_MAP[DataProvider.IDEX][keyword.toUpperCase()]
    unreachable(dataProvider)
}
...
export function resolveCoinAddress(id: string, dataProvider: DataProvider) {
    ...
    if (dataProvider === DataProvider.IDEX) return ID_ADDRESS_MAP[DataProvider.IDEX][id]
    unreachable(dataProvider)
}
