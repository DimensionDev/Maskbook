...
import { PluginGammeRPC } from '../messages'
...
export function useMintCallback(from: string, stateType: US_STATE_TYPE, candidateType: CANDIDATE_TYPE) {
    ...
    const electionTokenContract = useTokenContract()
    ...
    const mintCallback = useCallback(async () => {
        ...
        try {
            const { mint_transaction_hash } = await PluginGammeRPC.mintElectionPacket(from, stateType, candidateType)
