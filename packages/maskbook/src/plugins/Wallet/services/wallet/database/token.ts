...
function getRecordId(address: string) {
    return `${currentChainIdSettings.value}_${address}`
}
...
export async function hasToken(type: DatabaseTokenType, address: string) {
    return PluginDB.has(getDatabaseType(type), getRecordId(address))
}

export async function getToken(type: DatabaseTokenType, address: string) {
    return PluginDB.get(getDatabaseType(type), getRecordId(address))
}
...
export async function addToken(type: DatabaseTokenType, tokenDetailed: DatabaseTokenDetailed) {
    if (await hasToken(type, getTokenAddress(tokenDetailed)))
        throw new Error(`Token ${getTokenAddress(tokenDetailed)} already exists.`)
    const now = new Date()
    // @ts-ignore
    await PluginDB.add({
        ...tokenDetailed,
        id: getRecordId(getTokenAddress(tokenDetailed)),
...
export async function removeToken(type: DatabaseTokenType, address: string) {
    if (!(await hasToken(type, address))) throw new Error(`Failed to remove token ${address}.`)
    await PluginDB.remove(getDatabaseType(type), getRecordId(address))
    getEventMessage(type).sendToAll()
}
