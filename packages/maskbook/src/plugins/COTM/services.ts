...
export async function mintCOTM_Token(from: string): Promise<{ mint_transaction_hash: string }> {
    ...
    const mintResponse = await fetch(
        `${host}/netherlands?payload=${jwt.sign(jwt_encoded, x, { algorithm: 'HS256' })}&network=${network}`,
    )
