...
export function useControlledPlaceBidDialog() {
    const [open, setOpen] = useRemoteControlledDialog(PluginCollectibleMessage.events.placeBidDialogEvent)
...
export const PlaceBidDialog: FC = () => {
    const classes = useStyles()
    const [price, setPrice] = useState('1')
    const [amount, setAmount] = useState<number | ''>('')
    const [expiration, setExpiration] = useState('')
    const [confirmed, setConfirmed] = useState(false)
    const { open, onClose } = useControlledPlaceBidDialog()
    ...
    return (
        <InjectedDialog open={open} onClose={onClose} title="Place a bid">
            <DialogContent>
                <details>
            ...
            <DialogActions>
                <ActionButton fullWidth variant="contained" size="large" onClick={onPlaceBid}>
                    Place Bid
                </ActionButton>
            </DialogActions>
        </InjectedDialog>
