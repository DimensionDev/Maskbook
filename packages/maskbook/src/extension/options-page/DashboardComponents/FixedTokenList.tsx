...
export function FixedTokenList(props: FixedTokenListProps) {
    ...
    const filteredTokens = erc20TokensDetailed.filter(
        (x) =>
            (!includeTokens.length || includeTokens.some(MatchAddress(x.address))) &&
            (!excludeTokens.length || !excludeTokens.some(MatchAddress(x.address))),
    )
        ...
        <FixedSizeList
            ...
            itemData={{
                ...
                onSelect(address: string) {
                    ...
                    const token = renderTokens.find((token) => matchAddress(token.address))
                    if (!token) return
