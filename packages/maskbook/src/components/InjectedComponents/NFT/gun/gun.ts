...
export async function getNFTAvatarFromGun(userId: string) {
...
async function getNFTAvatars() {
    ...
    const resultPromise = NFTAvatarKeys.map((key) => getNFTAvatarFromGun(key))
    const result = (await Promise.all(resultPromise)).filter((x) => x) as AvatarMetaDB[]
    return result
}

export async function saveNFTAvatarGun(userId: string, avatarId: string, address: string, tokenId: string) {
