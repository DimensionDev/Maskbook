...
import type { NFTRSS3Node } from '../types.js'
...
export function useGetNFTAvatarFromRSS3() {
    return useCallback(async (userId: string, address: string, key: RSS3_KEY_SITE) => {
        ...
        const result = await rss3Storage.get<Record<string, NFTRSS3Node>>(key)
        if (result) return result[userId].nft

        return (await rss3Storage.get<NFTRSS3Node>('_nft'))?.nft
    }, [])
}
