...
export function useTrade(inputToken?: Web3Helper.FungibleTokenAll, outputToken?: Web3Helper.FungibleTokenAll) {
    ...
    return useAsyncRetry(async () => {
        ...
        if (!Utils.isNativeTokenSchemaType(inputToken.schema) && !Utils.isNativeTokenSchemaType(outputToken.schema))
            return false
        ...
        if (!isSameAddress(inputToken.address, WNATIVE_ADDRESS) && !isSameAddress(outputToken.address, WNATIVE_ADDRESS))
            return false
        return true
    }, [WNATIVE_ADDRESS, inputToken, outputToken, pluginID, Utils.isNativeTokenSchemaType])
}
