...
export function useWarnings(formChainId: number, formSymbol?: string) {
    ...
    const chainIdWarning = useMemo(() => {
        ...
        return t`This Chain ID is currently used by the ${duplicated.name} network. `
    ...
    const symbolWarning = useMemo(() => {
        ...
        if (match.nativeCurrency.symbol !== formSymbol)
            return t`The network with chain ID ${formChainId} may use a different currency symbol (${match.nativeCurrency.symbol}) than the one you have entered. Please check.`
        return undefined
