...
import * as gun from /* webpackDefer: true */ '@masknet/gun-utils'
...
export async function GUN_queryPostKey_version40(
    ...
    const result = await gun.getGunData('maskbook', 'posts', encodeArrayBuffer(iv), whoAmI)
    ...
    export async function* GUN_queryPostKey_version39Or38(
        ...
        const internalNodeNames = uniq(
            (
                await Promise.all([
                    //
                    gun.getGunData(postHash[0], keyHash),
                    gun.getGunData(postHash[1], keyHash),
                ])
            )
                .filter(isNonNull)
                .filter(isObject)
        ...
        const resultPromise = internalNodeNames.map((key) => gun.getGunData(key))
        ...
        const iter = new EventIterator<DecryptStaticECDH_PostKey>((queue) => {
            ...
            // future results
            Promise.all([
                main(gun.subscribeGunMapData([postHash[1]], isValidData, abortSignal)),
                main(gun.subscribeGunMapData([postHash[0]], isValidData, abortSignal)),
            ]).then(() => queue.stop())
    ...
    export async function publishPostAESKey_version39Or38(
        ...
        for (const result of receiversKeys.values()) {
            try {
                ...
                gun.pushToGunDataArray([postHash, keyHash], post)
            } catch (error) {
...
namespace Version37 {
    export async function* GUN_queryPostKey_version37(
        ...
        const internalNodeNames = await gun.getGunData(keyPartition).then((x) => {
            if (!x) return []
        ...
        const iter = new EventIterator<DecryptEphemeralECDH_PostKey>((queue) => {
            ...
            // future results
            main(gun.subscribeGunMapData([keyPartition], isValidData, abortSignal))
    ...
    export async function publishPostAESKey_version37(
        ...
        for (const result of receiversKeys.values()) {
            try {
                ...
                gun.pushToGunDataArray([keyPartition], post)
            } catch (error) {
                console.error('[@masknet/encryption] An error occurs when sending E2E keys', error)
            }
        }
