...
const methods = {
    ...
    async wallet_requestPermissions(request: EIP2255PermissionRequest) {
        ...
        if (Object.keys(request).length === 0)
            throw new E(C.InvalidParams, M.wallet_requestPermissions_Empty.replace('$', location.origin))
        for (const key in request) {
            if (typeof key !== 'string' || typeof request[key] !== 'object' || request[key] === null)
                throw new E(C.InvalidParams, M.wallet_requestPermissions_Unknown.replace('$', location.origin))
        }
...
export async function eth_request(request: unknown) {
    ...
    if (!(method in methods)) throw new E(C.MethodNotFound, M.UnknownMethod.replace('$', method))
