...
import { useContract, useRedPacketConstants, ChainId, useChainId } from '@masknet/web3-shared-evm'
import type { AbiItem } from 'web3-utils'

export function useRedPacketContract(version: number, chainId?: ChainId) {
    ...
    const v1 = useContract<HappyRedPacketV1>(chainId ?? _chainId, addressV1, HappyRedPacketV1ABI as AbiItem[])
    const v2 = useContract<HappyRedPacketV2>(chainId ?? _chainId, addressV2, HappyRedPacketV2ABI as AbiItem[])
    const v3 = useContract<HappyRedPacketV3>(chainId ?? _chainId, addressV3, HappyRedPacketV3ABI as AbiItem[])
    const v4 = useContract<HappyRedPacketV4>(chainId ?? _chainId, addressV4, HappyRedPacketV4ABI as AbiItem[])
