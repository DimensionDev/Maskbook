...
import { useContract, useRedPacketConstants, ChainId } from '@masknet/web3-shared-evm'
...
export function useRedPacketContract(version: number, chainId?: ChainId) {
    ...
    const v1 = useContract<HappyRedPacketV1>(addressV1, HappyRedPacketV1ABI as AbiItem[], chainId)
    const v2 = useContract<HappyRedPacketV2>(addressV2, HappyRedPacketV2ABI as AbiItem[], chainId)
    const v3 = useContract<HappyRedPacketV3>(addressV3, HappyRedPacketV3ABI as AbiItem[], chainId)
    const v4 = useContract<HappyRedPacketV4>(addressV4, HappyRedPacketV4ABI as AbiItem[], chainId)
