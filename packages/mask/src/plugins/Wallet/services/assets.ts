    ...
    switch (provider) {
        case FungibleAssetProvider.ZERION:
            ...
            const scopes = network
                ...
                : ['assets', 'bsc-assets', 'polygon-assets', 'arbitrum-assets', 'avalanche-assets', 'optimistic-assets']
...
function formatAssetsFromZerion(
    ...
    return data.map(({ asset, quantity }) => {
        ...
        const isNativeToken = (symbol: string) => ['ETH', 'BNB', 'MATIC', 'ARETH', 'AETH', 'OETH'].includes(symbol)
