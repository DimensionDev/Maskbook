...
import type { PairToken, TokenDetail } from '../../../types'
...
export default class BenQiRewardProtocol extends CompoundTimestampBasedProtocol {
    ...
    public async getAllNeedFetchPricePairs() {
        ...
        const matchPairs = this.allPairs
            .filter((pair: PairToken) => pair[1]?.symbol && allTokens.find((item) => item.symbol === pair[1].symbol))
            .map((pair: PairToken) => {
                return {
                    underlying: pair[0],
                    market: pair[1],
