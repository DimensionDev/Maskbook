...
const Asset: FC<AssetProps> = ({ src, alt }) => {
    ...
    if (isVideo) {
        return (
            <img
                ...
                src={`https://shoyu.mypinata.cloud/ipfs/${path[0]}/${path[1]}/${path[2]}/nft.jpg`}
                alt={alt}
            />
        )
    }
    ...
    if (is3D) {
        return (
            <img
                className={classes.image}
                src={`https://shoyu.mypinata.cloud/ipfs/${path[0]}/${path[1]}/${path[2]}/nft.png?q=80`}
                alt={alt}
            />
        )
    }
...
function ShoyuImage(props: Props) {
    const { classes } = useStyles()
...
export default ShoyuImage

// back here: ref https://api.shoyunft.com/metadata/1/0x64b8b945b62cd5cf75d3b0b4bf64ec4aecb6b881/0.json
