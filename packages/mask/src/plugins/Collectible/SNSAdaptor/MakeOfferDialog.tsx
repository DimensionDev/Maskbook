...
import type { NetworkPluginID, NonFungibleAsset, rightShift, ZERO } from '@masknet/web3-shared-base'
...
export function MakeOfferDialog(props: MakeOfferDialogProps) {
    ...
    // const isAuction = asset?.is_auction ?? false
    // const isVerified = asset?.is_verified ?? false
    // const leastPrice =
    //     asset && asset.desktopOrder ? new BigNumber(asset.desktopOrder.current_price ?? '0') : ZERO

    // const paymentTokens = uniqBy(
    //     [...(asset?.offer_payment_tokens ?? []), ...(asset?.order_payment_tokens ?? [])],
    //     (x) => x.address,
    // )
    ...
    // const { amount, token, balance, setAmount, setAddress } = useFungibleTokenWatched(selectedPaymentToken)

    // const onMakeOffer = useCallback(async () => {
    //     if (!asset) return
    //     if (!asset.token_id || !asset.token_address) return
    //     if (!token?.value) return
    //     if (token.value.schema !== SchemaType.Native && token.value.schema !== SchemaType.ERC20) return
    //     const schemaName = asset.asset_contract?.schemaName
    ...
    //                             />
    //                             {(isAuction ? asset.isCollectionWeth : asset.isOrderWeth) ||
    //                             insufficientBalance ? (
    //                                 <ActionButton
    //                                     className={classes.button}
    //                                     variant="contained"
