...
import { isGreaterThan, isZero, NetworkPluginID, FungibleToken, calcBalance } from '@masknet/web3-shared-base'
...
export function CreateForm(props: CreateFormProps) {
    ...
    const [totalOfPerWallet, setTotalOfPerWallet] = useState(
        isZero(origin?.limit || '0') ? '' : calcBalance(origin?.limit || '0', origin?.token?.decimals).toFixed(),
    )
    ...
    if (origin?.token && origin?.total) {
        TAS.push({
            token: origin?.token,
            amount: calcBalance(origin?.total || '0', origin?.token.decimals).toFixed(),
            key: uuid(),
        })
    }
    if (origin?.exchangeTokens && origin?.exchangeAmounts) {
        origin?.exchangeTokens.map((i, x) =>
            TAS.push({
                amount: calcBalance(origin?.exchangeAmounts[x] || '0', i?.decimals).toFixed(),
                token: i,
                key: uuid(),
            }),
        )
