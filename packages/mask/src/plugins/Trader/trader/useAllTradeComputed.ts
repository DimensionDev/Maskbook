...
export function useAllTradeComputed(
    ...
    const {
        trader_: oolongswap_,
        trader: oolongswap,
        traderEstimateGas: oolongswapEstimateGas,
    } = useUniswapV2Like(tradeProviders, TradeProvider.OOLONGSWAP, inputAmount_, inputToken, outputToken)

    // SwapperChan
    const {
        trader_: swapperchan_,
        trader: swapperchan,
        traderEstimateGas: swapperchanEstimateGas,
    } = useUniswapV2Like(tradeProviders, TradeProvider.SWAPPERCHAN, inputAmount_, inputToken, outputToken)
