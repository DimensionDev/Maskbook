...
export function useAllTradeComputed(
    ...
    const {
        trader_: spiritswap_,
        trader: spiritswap,
        traderEstimateGas: spiritswapEstimateGas,
    } = useUniswapV2Like(tradeProviders, TradeProvider.SPIRITSWAP, inputAmount_, inputToken, outputToken)

    // Spookyswap
    const {
        trader_: spookyswap_,
        trader: spookyswap,
        traderEstimateGas: spookyswapEstimateGas,
    } = useUniswapV2Like(tradeProviders, TradeProvider.SPOOKYSWAP, inputAmount_, inputToken, outputToken)
