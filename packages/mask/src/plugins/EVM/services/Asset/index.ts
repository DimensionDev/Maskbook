...
import { NonFungibleTokenAPI, OpenSea, Rarible, Zora, Treasure } from '@masknet/web3-providers'
...
export async function getAsset(options: AssetOption) {
    switch (options.provider) {
        ...
        case NonFungibleAssetProvider.TREASURE:
            return Treasure.getAsset(options.address, options.tokenId, { chainId: options.chainId })
        default:
...
export async function getOrders(options: OrderOption) {
    ...
    switch (provider) {
        ...
        case NonFungibleAssetProvider.TREASURE:
            return Treasure.getOrders(address, tokenId, side, { chainId })
        default:
...
export async function getHistory(options: HistoryOption) {
    ...
    switch (provider) {
        ...
        case NonFungibleAssetProvider.TREASURE:
            return Treasure.getHistory(address, tokenId)
        default:
            unreachable(provider)
    }
}
