...
function saveTheBlurBird() {
    const logo = logoSelector().evaluate()

    if (logo?.parentElement) {
        // eslint-disable-next-line @masknet/browser-no-set-html
        logo.parentElement.innerHTML = BlueBirdHTML
    }
}

export function injectLogoAtTwitter(signal: AbortSignal) {
    const watcher = new MutationObserverWatcher(logoSelector()).addListener('onAdd', saveTheBlurBird)
