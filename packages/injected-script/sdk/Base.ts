...
export class InjectedProvider {
    ...
    connect(options: unknown): Promise<unknown> {
        return createRequest((id) => sendEvent('web3BridgeExecute', [this.pathname, 'connect'].join('.'), id, options))
    }
    ...
    disconnect(): Promise<void> {
        return createRequest((id) => sendEvent('web3BridgeExecute', [this.pathname, 'disconnect'].join('.'), id))
    }
    ...
    async untilAvailable(validator: () => Promise<boolean> = () => Promise.resolve(true)): Promise<void> {
        await createRequest((id) => sendEvent('web3UntilBridgeOnline', this.pathname.split('.')[0], id))
        if (await validator()) {
    ...
    request<T extends unknown>(data: unknown): Promise<T> {
        return createRequest((id) => sendEvent('web3BridgeExecute', [this.pathname, 'request'].join('.'), id, data))
    }
    ...
    getProperty(key: string): Promise<unknown> {
        return createRequest((id) => sendEvent('web3BridgePrimitiveAccess', this.pathname, id, key))
    }
}
