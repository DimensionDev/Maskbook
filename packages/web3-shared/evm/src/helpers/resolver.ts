...
import { isENSContractAddress, isENSWrapperContractAddress } from './address.js'
...
export function resolveImageURL(image?: string, name?: string, collectionName?: string, address?: string) {
    ...
    if (!address) return
    if (isENSContractAddress(address) || isENSWrapperContractAddress(address)) return ENS_IMAGE
    return
}
