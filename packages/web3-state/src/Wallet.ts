...
export class WalletState<ProviderType extends string, Transaction> implements Web3WalletState<Transaction> {
    ...
    async removeWallet(address: string, password?: string | undefined) {
        const source = this.storage.value?.[this.providerType]
        await this.storage.setValue({
            ...this.storage.value,
            [this.providerType]: source?.filter((x) => !isSameAddress(x.address, address)),
        })
    }
    ...
    async removeWallets(wallets: Wallet[]) {
        ...
        await this.storage.setValue({
            ...this.storage.value,
            [this.providerType]: source?.filter((x) => !wallets.find((y) => isSameAddress(x.address, y.address))),
        })
    }
