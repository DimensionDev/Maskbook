...
const IN_MEMORY_CACHE: {
    lastNetworkCacheKey?: string
    lastTransactionCacheKey?: string
...
function useSettingsContext(initial?: {
    ...
    const networkCacheKey = `${pluginID}_${chainId}`
    const transactionCacheKey = stringify([chainId, transactionOptions])
    const needToResetByNetwork =
        !!IN_MEMORY_CACHE.lastNetworkCacheKey && IN_MEMORY_CACHE.lastNetworkCacheKey !== networkCacheKey
    const needToResetByTransaction =
        !!IN_MEMORY_CACHE.lastTransactionCacheKey && IN_MEMORY_CACHE.lastTransactionCacheKey !== transactionCacheKey
    ...
    const onClearInMemoryCache = useCallback(() => {
        ...
        delete IN_MEMORY_CACHE.lastNetworkCacheKey
        delete IN_MEMORY_CACHE.lastTransactionCacheKey
    ...
    useEffect(() => {
        IN_MEMORY_CACHE.lastNetworkCacheKey = networkCacheKey
        IN_MEMORY_CACHE.lastTransactionCacheKey = transactionCacheKey
        IN_MEMORY_CACHE.lastSelectedGasOptionType = gasOptionType
        IN_MEMORY_CACHE.lastSelectedGasSettingsType = gasSettingsType
    }, [gasOptionType, gasSettingsType, networkCacheKey, transactionCacheKey])
