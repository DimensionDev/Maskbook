...
function deconstructAlpha40(str: string, throws = false): PayloadAlpha40 | null {
    ...
    if (!payloadStart)
        if (throws) throw new Error(geti18nString('payload-not-found'))
        else return null
    const [payload, rest] = payloadStart.split(':||')
    if (rest === undefined)
        if (throws) throw new Error(geti18nString('payload-incomplete'))
        else return null
    const [ownersAESKeyEncrypted, iv, encryptedText, signature, ...extra] = payload.split('|')
    if (!(ownersAESKeyEncrypted && iv && encryptedText))
        if (throws) throw new Error(geti18nString('payload-bad'))
        else return null
...
function deconstructAlpha41(str: string, throws = false): null | never {
    ...
    if (str.match('🎼1/4') && str.match(':||'))
        if (throws) throw new Error(geti18nString('payload-throw-in-alpha41'))
        else return null
    return null
}
...
export function deconstructPayload(str: string, throws = false): PayloadAlpha40 | null {
    ...
    if (str.match('🎼') && str.match(':||'))
        if (throws) throw new TypeError(geti18nString('service-unknown-payload'))
        else return null
    if (throws) throw new TypeError(geti18nString('payload-not-found'))
    else return null
}
