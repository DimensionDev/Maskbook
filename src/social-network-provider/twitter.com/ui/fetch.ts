...
import { bioCardParser, postParser, postIdParser, postImageAttachmentsParser } from '../utils/fetch'
...
function collectPostInfo(tweetNode: HTMLDivElement | null, info: PostInfo, self: Required<SocialNetworkUIDefinition>) {
    ...
    untilElementAvailable(postsImageSelector(tweetNode), 10000)
        .then(() => postImageAttachmentsParser(tweetNode))
        .then((attachments) => {
            console.log('DEBUG: post attachments')
