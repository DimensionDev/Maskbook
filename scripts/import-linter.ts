...
const background = project.getSourceFileOrThrow(path.join(__dirname, '../packages/maskbook/src/background-service.ts'))

const checkedList = new Set<SourceFile>()
checkedList.add(project.getSourceFileOrThrow(path.join(__dirname, '../packages/maskbook/src/utils/safeRequire.ts')))
